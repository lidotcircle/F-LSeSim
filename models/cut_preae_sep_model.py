...
from models.pregan_networks import ResnetSepGenerator
...
class CUTPreAESepModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        parser.add_argument('--AE_freq', type=int, default=5, help='include kld of translated cases')
        parser.add_argument('--lambda_GAN', type=float, default=1.0, help='weight for GAN loss：GAN(G(X))')
        parser.add_argument('--lambda_AE', type=float, default=5.0, help='weight for AE reconstruction loss')
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['G_GAN', 'D_real', 'D_fake', 'G', 'NCE', 'AE', 'focus']
        ...
        if opt.netG != 'resnet_preae_sep':
            raise NotImplemented(f'{opt.netG} is not implemented for CUT_PRE model')
        netG: ResnetSepGenerator = networks.define_G(opt.input_nc, opt.output_nc, opt.ngf, opt.netG, opt.normG, not opt.no_dropout, opt.init_type, opt.init_gain, opt.no_antialias, opt.no_antialias_up, self.gpu_ids, opt)
        self.netG = netG
        self.transMode = lambda: netG.setTransMode()
        self.aeMode = lambda: netG.setAEMode()
        ...
        if self.isTrain:
            ...
            self.optimizer_G = torch.optim.Adam(self.netG.TransParameters(), lr=opt.lr * opt.gen_lr_multiplier, betas=(opt.beta1, opt.beta2))
            self.optimizer_AE = torch.optim.Adam(self.netG.AEParameters(), lr=opt.lr, betas=(opt.beta1, opt.beta2))
    ...
    def optimize_parameters(self):
        ...
        if self.optimize_itern < 500 or self.optimize_itern % self.opt.AE_freq == 0:
            # update AE
            self.aeMode()
        ...
        self.optimize_itern = self.optimize_itern + 1
    ...
    def compute_G_loss(self):
        ...
        self.loss_G = self.loss_G_GAN + loss_NCE_both * self.opt.lambda_NCE + self.loss_focus * self.opt.lambda_NO
        return self.loss_G

    def compute_rec(self):
        rec = self.netG(self.real, None)
    ...
    def compute_AE_loss(self):
        ...
        loss_AE = 0.5 * (loss_A_recon + loss_B_recon)
        return loss_AE
