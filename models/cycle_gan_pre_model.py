...
class CycleGANPreModel(BaseModel):
    @staticmethod
    ...
    def __init__(self, opt):
        ...
        if opt.netG != 'resnet_pre':
            raise NotImplemented(f'not implemented module {opt.netG}')
        ...
        if self.isTrain:
            ...
            if self.learned_feature:
                self.optimizer_Pre = torch.optim.Adam(itertools.chain(self.netPre.parameters()), lr=opt.lr, betas=(opt.beta1, 0.999))
    ...
    def forward(self):
        ...
        self.fake_B, _ = self.netG_A(self.real_A, self.feat_real_A)  # G_A(A)
        ...
        self.fake_A, _ = self.netG_B(self.real_B, self.feat_real_B)  # G_B(B)
    ...
    def backward_G(self):
        ...
        if lambda_idt > 0:
            ...
            self.idt_A, _ = self.netG_A(self.real_B, self.feat_real_B)
            self.loss_idt_A = self.criterionIdt(self.idt_A, self.real_B) * lambda_B * lambda_idt
            # G_B should be identity if real_A is fed: ||G_B(A) - A||
            self.idt_B, _ = self.netG_B(self.real_A, self.feat_real_A)
    ...
    def compute_visuals(self):
        ...
        if hasattr(self, 'netD_A'):
            ...
            self.val_loss_G_A = self.criterionGAN(pred_fake_B, True)
            self.val_loss_D_A_real = self.criterionGAN(pred_real_B, True)
            self.val_loss_D_A_fake = self.criterionGAN(pred_fake_B, False)
        if hasattr(self, 'netD_B'):
            pred_fake_A = self.netD_B(self.fake_A)
            pred_real_A = self.netD_B(self.real_A)
            self.val_loss_G_B = self.criterionGAN(pred_fake_A, True)
            self.val_loss_D_B_real = self.criterionGAN(pred_real_A, True)
            self.val_loss_D_B_fake = self.criterionGAN(pred_fake_A, False)