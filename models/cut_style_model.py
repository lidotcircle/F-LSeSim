...
class CUTStyleModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        if opt.netG != 'transtyle':
            raise NotImplemented(f"{opt.netG} for cut_style")
    ...
    def forward(self):
        ...
        self.fake = self.netG(self.real, return_style=True)
    ...
    def compute_G_loss(self):
        ...
        if self.opt.lambda_Style > 0.0 and hasattr(self, 'styles_logit_A'):
            self.loss_Style_A = self.criterionStyle(self.styles_logit_A, True).mean()
            self.loss_Style_B = self.criterionStyle(self.styles_logit_B, False).mean()
